...
corr_p <- get_mat_plot(mean_corr_matrix, main = 'Pearson correlation', palette = "Greens")

paletteLength <- 100
myColor <- 
  colorRampPalette(c("whitesmoke","#AD4747"))(paletteLength)
...
for(cptac in names(venn_p)) {
  venn <- venn_p[[cptac]]
  venn$labels <- rep("", length(v$labels))
