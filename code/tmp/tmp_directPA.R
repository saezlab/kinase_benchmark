...
library("directPA")
install.packages("PhosR")
library("PhosR")
data("PhosphoSitePlus")

## load the proteomics dataset
data(PM)
## PM dataset is just normal data.frame with logFC/t values etc.

# load pathway annotations
data(Pathways)
kinase <- lapply(PhosphoSite.human, function(x){gsub(";[STY]", ";", x)})
...
dPA <- directPA(Tc=PM[,c(1,1)], direction=180, annotation=Pathways.reactome)
dPA$pathways %>% head()
dPA$gene.pvalues
