...
if(exists("snakemake")){
  ...
  input_file_raw <- list.files("results/hernandez/final_scores/raw", pattern = "GPSppsp", full.names = T)
  input_file_scaled <- list.files("results/hernandez/final_scores/scaled", pattern = "GPSppsp", full.names = T)
...
map(list(input_file_raw, input_file_scaled), function(input_file){
  if (any(str_detect(input_file, "raw"))){
    prior <- str_remove(str_remove(input_file, "results/hernandez/final_scores/raw/"), ".rds")
  } else if (any(str_detect(input_file, "scaled"))){
  ...
  map(prior, function(prior_i){
    ...
    if (any(str_detect(input_file, "raw"))){
      ...
      saveRDS(score_list_wide, paste0("results/hernandez/final_scores/scaled_subset/", prior_i, ".rds"))
    }
  })
