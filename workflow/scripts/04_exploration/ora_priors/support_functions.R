...
{  
  ...
  print(head(data.to.plot))
  comp1 <- ggplot(data.to.plot, aes(x = pc1, color = condition)) + geom_density() + theme_minimal() +
  ...
  comp2 <- ggplot(data.to.plot, aes(x = pc2, color = condition)) + geom_density() + theme_minimal() +
  ...
  comp3 <- ggplot(data.to.plot, aes(x = pc3, color = condition)) + geom_density() + theme_minimal() +
                          ...
                          label = FALSE, straight = FALSE, nlabels, manual_labels = NA)
{
  ...
  df$couleur <- ifelse(df$couleur == "1" & df$logFC < 0, "2",
                       df$couleur)
  if (label) {
    a <- ggplot(df, aes(x = logFC, y = -log10(adj.P.Val), color = couleur)) +
...
#'
#' @param entrez_ID vector of entrez_ID
...
convert_genesymbols_to_entrezid <- function(entrez_ID){
  
  map_table <- AnnotationDbi::mapIds(org.Hs.eg.db::org.Hs.eg.db, entrez_ID, 'SYMBOL', 'ENTREZID')
  
  stopifnot(length(map_table) == length(entrez_ID))
  stopifnot(all(names(map_table) == entrez_ID))
...
lineplot_time_conc <- function(t_table_long, drug, feature_to_plot)
{
  max_abs <- max(abs(t_table_long$value), na.rm = T)
  max_abs <- max_abs + 0.2 * max_abs
  
  t_table_long_drug <- t_table_long[t_table_long$drug == drug,]
  
  
  t_table_long_drug_psite <- t_table_long_drug[t_table_long_drug$ID == feature_to_plot,]
  
  
  t_table_long_drug_psite$time_h <- as.numeric(gsub("h","",t_table_long_drug_psite$time))
  ggplot_result <- ggplot(t_table_long_drug_psite, aes(x = time_h, y = value, group = concentration, color = concentration)) + 
    geom_line() + 
    geom_point(alpha = 0.5, size = 2.5) + 
    ylim(c(-max_abs,max_abs)) +
    theme_classic() + 
    ggtitle(paste(gsub("___.*","",feature_to_plot), drug, sep = "_drug:")) +
...
write_KSN_subset <- function(kinase_names, kinase_activities_df, phospho_diff_df, KSN, ouput_path){
    KSN_subset <- KSN[which(KSN$source %in% kinase_names),]
    KSN_subset <- KSN_subset[which(KSN_subset$target %in% phospho_diff_df[,1]),]
    
    kinase_activities_df_subset <- kinase_activities_df[which(kinase_activities_df[,1] %in% kinase_names),]
    kinase_activities_df_subset$nodetype <- "kinase"
    
    phospho_diff_df_subset <- phospho_diff_df[which(phospho_diff_df[,1] %in% KSN$target),]
    phospho_diff_df_subset$nodetype <- "psite"
    
    ATT <- as.data.frame(rbind(kinase_activities_df_subset, phospho_diff_df_subset))
    SIF <- KSN_subset
    
    write_csv(ATT, file = paste(ouput_path, paste(paste(kinase_names,collapse = "_"), "_ATT_filtered.csv",sep = ""),sep = ""))
    write_csv(SIF, file = paste(ouput_path, paste(paste(kinase_names,collapse = "_"), "_SIF_filtered.csv",sep = ""),sep = ""))
    ...
    write_csv(ATT, file = paste(ouput_path, paste(paste(kinase_names,collapse = "_"), "_ATT_unfiltered.csv",sep = ""),sep = ""))
    write_csv(SIF, file = paste(ouput_path, paste(paste(kinase_names,collapse = "_"), "_SIF_unfiltered.csv",sep = ""),sep = ""))
}

display_KSN_stats <- function(kinase_names, kinase_activities_df, phospho_diff_df, KSN){
  KSN_subset <- KSN[which(KSN$source %in% kinase_names),]
  KSN_list <- sapply(kinase_names,function(kinase){
    # print(paste(kinase, paste(" number of targets: ",length(KSN_subset[which(KSN_subset$source == kinase),1]),sep = ""), sep = ""))
    return(KSN_subset[which(KSN_subset$source == kinase),2])
    }, USE.NAMES = T)
  
  print("WITHOUT FILTERING BY MEASURED TARGETS:")
  print(make_comb_mat(KSN_list))
  plot(UpSet(make_comb_mat(KSN_list)))
  
  KSN_subset <- KSN_subset[which(KSN_subset$target %in% phospho_diff_df[,1]),]
  KSN_list <- sapply(kinase_names,function(kinase){
    # print(paste(kinase, paste(" number of measured targets: ",length(KSN_subset[which(KSN_subset$source == kinase),1]),sep = ""), sep = ""))
    return(KSN_subset[which(KSN_subset$source == kinase),2])
  }, USE.NAMES = T)
  
  print("AFTER FILTERING BY MEASURED TARGETS:")
  print(make_comb_mat(KSN_list))
  
  plot(UpSet(make_comb_mat(KSN_list)))
}

heatmap_KSN_sources <- function(KSN){
  KSN_CDK_sources <- unlist(sapply(KSN$sources,function(x){strsplit(x,";")}))
  table(KSN_CDK_sources)
  KSN_CDK_source_heatmap <- as.data.frame(matrix(nrow = length(KSN[,1]),ncol = length(table(KSN_CDK_sources))))
  names(KSN_CDK_source_heatmap) <- names(table(KSN_CDK_sources))
  ressources_KSN <- names(KSN_CDK_source_heatmap)
  KSN_CDK_source_heatmap$psites <- paste(KSN$enzyme_genesymbol, paste(KSN$substrate_genesymbol, paste(KSN$residue_type, KSN$residue_offset, sep = ""),sep = "_"), sep = "___")
  KSN_CDK_source_heatmap <- KSN_CDK_source_heatmap[!duplicated(KSN_CDK_source_heatmap$psites),]
  row.names(KSN_CDK_source_heatmap) <- KSN_CDK_source_heatmap$psites
  KSN_CDK_source_heatmap <- KSN_CDK_source_heatmap[,-which(names(KSN_CDK_source_heatmap) == "psites")]
  
  for(i in 1:length(KSN_CDK_source_heatmap[,1]))
  {
    KSN_CDK_current_sources <- KSN[i,"sources",drop = F]
    KSN_CDK_source_heatmap[i,] <- sapply(names(KSN_CDK_source_heatmap),function(x,KSN_CDK_current_sources){
      sum(grepl(x,KSN_CDK_current_sources))
    },KSN_CDK_current_sources= KSN_CDK_current_sources) 
  }
  pheatmap::pheatmap(KSN_CDK_source_heatmap, show_rownames = F, angle_col = 315)
}

overlap_targets_heatmap <- function(kinase_names,t_table,KSN, row_order_by_sets = T,separation_of_columns = c())
{
  KSN_subset <- KSN[which(KSN$source %in% kinase_names),]
  KSN_list <- sapply(kinase_names,function(kinase){
    return(KSN_subset[which(KSN_subset$source == kinase),2])
  }, USE.NAMES = T)
  comb_mat <- make_comb_mat(KSN_list)
  combinaison_codes <- comb_name(comb_mat)
  
  combined_annotations <- sapply(combinaison_codes,function(combinaison, comb_mat){
    extract_comb(m = comb_mat, comb_name = combinaison)
  }, comb_mat = comb_mat)
  
  names(combined_annotations) <- sapply(names(combined_annotations),function(x,kinase_names){
    return(paste0(kinase_names[as.logical(as.numeric(strsplit(x,"")[[1]]))], collapse = "_"))
  }, kinase_names = kinase_names, simplify = T)
  
  combined_annotations <- stack(combined_annotations)
  row.names(combined_annotations) <- combined_annotations[,1]
  combined_annotations <- combined_annotations[,-1,drop = F]
  names(combined_annotations)[1] <- "kinase_target_overlap"
  
  t_table_subset <- t_table[which(t_table$ID %in% KSN_subset$target),]
  if(!row_order_by_sets)
  {
    t_table_to_clust <- t_table_subset[,-1]
    t_table_to_clust[is.na(t_table_to_clust)] <- 0
    clust_order <- hclust(d = dist(t_table_to_clust))$order
    
    t_table_subset <- t_table_subset[clust_order,]
  } else
  {
    combined_annotations <- combined_annotations[which(row.names(combined_annotations) %in% row.names(t_table_subset)),,drop = F]
    t_table_subset <- t_table_subset[row.names(combined_annotations),]
  }

  t <- as.vector(t(t_table_subset[,-1]))
  ...
  pheatmap::pheatmap(t_table_subset[,-1],color = palette, cluster_rows = F, annotation_row = combined_annotations, angle_col = 315, cluster_cols = F, gaps_col = separation_of_columns)
}

gene_and_comnplexes_is_expressed <- function(x, expressed_genes){
  if(grepl("_", x))
  {
    genes <- unlist(strsplit(x, "_"))
    if (sum(genes %in% expressed_genes) != 
        length(genes)) {
      return(NA)
    } else 
    {
