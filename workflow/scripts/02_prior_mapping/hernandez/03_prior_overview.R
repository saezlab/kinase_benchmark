if(exists("snakemake")){
  ...
  coverage_pdf <- "results/hernandez/overview_priors/coverage_kinases.pdf"
  edges_coverage_pdf <- "results/hernandez/overview_priors/coverage_edges.pdf"
  pps_coverage_pdf <- "results/hernandez/overview_priors/coverage_pps.pdf"
  height <- 6
  width <- 13
}
...
names(prior) <- str_remove(str_remove(prior_files, "results/hernandez/prior/"), ".tsv")
...
write_csv(coverage, overview_file)
