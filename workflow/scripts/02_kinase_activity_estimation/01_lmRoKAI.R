if(exists("snakemake")){
  ...
  output_file <- "results/02_activity_scores/hernandez/lmRoKAI/ptmsigdb.csv"
  remove_auto <- T
  scripts_method <- "workflow/scripts/methods/run_lm_rokai.R"
...
results <- future_map_dfr(1:ncol(phospho), function(i){
  ...
  run_lm_rokai(mat_i, prior, minsize = minsize)
})
