if(exists("snakemake")){
  ...
  output_file <- "results/02_activity_scores/hernandez/INKA/ptmsigdb.csv"
  remove_auto <- T
  scripts_method <- "workflow/scripts/methods/run_INKA.R"
...
results <- future_map_dfr(1:ncol(phospho), function(i){
  ...
  run_INKA(mat_i, prior, kinase_mapping = F, minsize = minsize)
})
